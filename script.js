document.addEventListener('DOMContentLoaded', () => {
  ...
  const queuePlural = document.getElementById('queue-plural');
  ...
  function setTheme(theme) {
    ...
    document.querySelector('meta[name="theme-color"]')?.setAttribute('content', isDark ? '#151229' : '#dfff69');
  ...
  function updateQueueControls() {
    const count = fileRecords.size;
    if (queueCount) queueCount.textContent = String(count);
    if (queuePlural) queuePlural.textContent = count === 1 ? '' : 's';
    queue?.classList.toggle('hidden', count === 0);
  ...
  function createFileItem(file) {
    ...
    status.textContent = 'Waking up the bats…';
  ...
  function markComplete(record, objectUrl, outputName, pageCount) {
    ...
    setProgress(record, 100, `Done. ${pageCount} ${pageCount === 1 ? 'page' : 'pages'} successfully de-flashbanged.`);
    ...
    download.title = 'Yoink your dark PDF';
  ...
  async function processFile(record) {
    if (typeof PDFLib === 'undefined') {
      markError(record, 'The PDF spellbook failed to load. Refresh while online and try again.');
      return;
    }
    ...
    try {
      setProgress(record, 8, 'Opening the forbidden document…');
      const fileBuffer = await record.file.arrayBuffer();
      const pdfDocument = await PDFDocument.load(fileBuffer);
      const pages = pdfDocument.getPages();

      if (pages.length === 0) throw new Error('This PDF contains zero pages. Impressive.');
      ...
      for (let index = 0; index < pages.length; index += 1) {
        ...
        setProgress(record, percent, `Teaching page ${index + 1} of ${pages.length} to fear daylight…`);
        if (index % 6 === 0) await yieldToBrowser();
      }

      setProgress(record, 90, 'Packing your newly nocturnal PDF…');
      ...
      const details = error instanceof Error && error.message ? error.message : 'Unknown PDF weirdness.';
      markError(record, `Could not convert it: ${details}`);
    }
  }
  ...
  async function handleFiles(fileCollection) {
    ...
    if (pdfFiles.length === 0) {
      showNotice('Those are not PDFs. The bats refuse to eat them.');
      return;
    }

    if (rejectedCount > 0) {
      showNotice(`${rejectedCount} non-PDF ${rejectedCount === 1 ? 'file was' : 'files were'} politely ignored.`);
    }
